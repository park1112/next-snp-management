...
## 버전 히스토리
- **v0.0.15**  
   ** 제일 처음 해야될것, 기존의 모든 타잎들을 정리하여, 구조에 맞도록 변수명과, type설정을 types폴더에서 관리하고 모든곳에서 가져와서 사용할수 있도록 하고 싶어, 
   그리고 파이어베이스에서 데이터 변수명과, 테이터구조가 뒤죽박죽인데, 이것을 좀더 효과적이고 체계적으로 적용해주었으면 좋겠어. 지금은 농가관리, 농지관리, 작업자관리, 작업일정관리, 계약관리, 정산관리만 있지만, 앞으로 농지에서 가져온 
   상품들을 창고에 저장하는 창고관리, 재고관리, 출고관리, 자제관리(바렛트, 양파망, 등등 같은 제품을 입출고), 까지 확장할 예정이야. 그러면 좀더 데이터구조를 잘 정돈해야될것같아. 
   그것이 완료 된후에 아래의 작업들을 추가로 작업해줘 
